...
object Lexer extends Pipeline[List[File], Iterator[Token]]
  ...
  val lexer = Lexer(
    ...
    word("/*") ~ many(elem(_ != '*')) ~ many1(elem('*')) ~ many(elem(c => c != '/' && c != '*') ~ many(elem(_ != '*')) ~ many1(elem('*'))) ~ word("/")
      |> {cs => CommentToken(cs.mkString(""))},
    //Unclosed multiline comment
    word("/*") ~ many(elem(_ != '*')) ~ many(elem(c => c != '/' && c != '*') ~ many(elem(_ != '*')) ~ many1(elem('*')))
      |> {(_,range) => ErrorToken("Error: unclosed multiline comment. Missing '*/'").setPos(range._1)}
